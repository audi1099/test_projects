    ...
    carDatabase.forEach((car, carIndex) => {
        car.records.forEach((record, recordIndex) => {
            ...
            row.innerHTML = `
                ...
                <td contenteditable="true" class="editable-description" data-car-index="${carIndex}" data-record-index="${recordIndex}">
                  ${record.description}
                </td>  
                <td>
...
elements.carTableBody.addEventListener('click', e => {
    ...
    if (e.target.classList.contains('edit')) {
        const newHours = prompt("Введите новые нормо-часы:", carDatabase[carIndex].records[recordIndex].hours)
